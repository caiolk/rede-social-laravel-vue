<template>
    ...
    <site-template>
      ...
      <span slot="principal">
        ...
        <grid-vue tamanho="12">
        <card-amigo-vue v-for="item in listaSeguidores" :key="item.id"
          :id="item.id"
          :usuarioid="item.id"
          :perfil="item.image" 
          :nome="item.name"
  ...
  methods:{
    ...
    atualizaPagina(){
      ...
      if(usuarioAux){
        ...
        this.$http.get(this.$urlAPI+`usuario/listaseguidores/`+this.$route.params.id,{"headers":{"Authorization":"Bearer "+this.$store.getters.getToken}})
        .then(response =>{
        
          
          this.listaSeguidores = response.data.seguidores;
